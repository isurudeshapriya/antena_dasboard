...
# --- Remove Selected Items Feature ---
def remove_selected_items(df, data_type):
    if df.empty:
        return df

    st.subheader(f"Remove Items from {data_type}")

    # Let user select rows to remove
    # We'll use a combination of Model + Project + Batch as a unique key
    df["UniqueKey"] = df["Model"].astype(str) + " | " + df["Project"].astype(str) + " | " + df["Batch"].astype(str)
    options = df["UniqueKey"].tolist()
    ...
    if st.button(f"❌ Remove Selected Items from {data_type}"):
        if selected_to_remove:
            df = df[~df["UniqueKey"].isin(selected_to_remove)].copy()
            df.drop(columns=["UniqueKey"], inplace=True)
            st.success(f"Removed {len(selected_to_remove)} item(s) from {data_type}")

            # Update session state and save to Excel
            if data_type == "Antenna":
                st.session_state.antenna_data = df
                st.session_state.antenna_data.to_excel(antenna_file_path, index=False)
            else:
            ...
            st.warning("No items selected to remove.")

    # Drop the temporary key column if user hasn't removed yet
    if "UniqueKey" in df.columns:
        df.drop(columns=["UniqueKey"], inplace=True)

    return df

# --- Call this after displaying the table ---
if st.session_state.selected_type:
    df_to_show = get_filtered_data(df)
    st.subheader(f"{st.session_state.selected_type} Data Table")
    display_table(df_to_show, st.session_state.selected_type)
    ...
    updated_df = remove_selected_items(df_to_show, st.session_state.selected_type)
